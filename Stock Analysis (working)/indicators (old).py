        ...
        if mktCap >= 1000000: #checks if bigger than or equal to 1 million
            if mktCap >= 1000000000: #checks if bigger than or equal to 1 billion
                if mktCap >= 1000000000000: #checks if bigger than or equal to 1 trillion
                    return (str(mktCap/1000000000000)+"T") #divides number by a trillion and append a “T”
                return (str(mktCap/1000000000)+"B") #divides number by a billion and append a “B”
            return (str(mktCap/1000000)+"M") #divides number by a million and append a “M”
        return (str(mktCap/1000)+"k") #divides number by a thousand and append a “k”
    return (str(mktCap)) #just returns market capitalisation
...
def eval(arr):
    length = len(arr)
    counter = 0
    SMA10 = fastSMA(arr[(length-10):],10,False)
    SMA50 = fastSMA(arr[(length-50):],50,False)
    SMA100 = fastSMA(arr[(length-100):],100,False)
    EMA10 = EMA(arr[(length-10):],10,False)
    EMA50 = EMA(arr[(length-10):],50,False)
    EMA100 = EMA(arr[(length-10):],100,False)
    RSICalc = RSI(arr[(length-15):])
    MACDCalc = MACD(arr[length-34:],False)
